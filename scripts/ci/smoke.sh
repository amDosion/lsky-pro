...
PHP_BIN="${PHP_BIN:-php}"
DB_FILE="${DB_FILE:-$ROOT_DIR/database/smoke-ci.sqlite}"
...
find app config database routes tests scripts -type f -name '*.php' -print0 | xargs -0 -n1 "$PHP_BIN" -l >/dev/null
...
run_env "$PHP_BIN" artisan test tests/Feature/Smoke tests/Feature/Api --stop-on-failure
