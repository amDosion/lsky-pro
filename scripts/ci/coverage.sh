...
PHP_BIN="${PHP_BIN:-php}"
DB_FILE="${DB_FILE:-$ROOT_DIR/database/smoke-coverage.sqlite}"
...
if ! "$PHP_BIN" -m | grep -qi xdebug; then
  echo "[FAIL] xdebug is required for coverage collection"
  exit 1
fi
...
run_env "$PHP_BIN" artisan test tests/Feature/Smoke tests/Feature/Api --coverage-clover=storage/coverage/clover.xml --coverage-text

echo "[OK] coverage report: storage/coverage/clover.xml"
