...
### 运维执行入口（Project Local Docs）
- 执行清单：`docs/tasks/TASKS.md`
- 文档索引与执行顺序：`docs/README.md`
- 运维闭环总手册：`docs/runbook/OPERATIONS_RUNBOOK.md`
- 统一验证入口（P0 编排）：`bash scripts/run-all.sh`

### 首次启动自动初始化（推荐）
- 在 `.env` 中配置数据库与管理员变量：
  - `DB_CONNECTION/DB_HOST/DB_PORT/DB_DATABASE/DB_USERNAME/DB_PASSWORD`
  - `INIT_AUTO_BOOTSTRAP=true`
  - `INIT_ADMIN_NAME/INIT_ADMIN_EMAIL/INIT_ADMIN_PASSWORD`
- 执行：
  - `php artisan lsky:bootstrap --force`
- 作用：
  - 自动执行迁移
  - 自动初始化基础配置/默认组/默认策略（缺失时）
  - 自动创建或修复管理员账号
  - 自动生成 `installed.lock`
